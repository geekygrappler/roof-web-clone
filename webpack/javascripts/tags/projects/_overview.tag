<r-project-overview>
  ...
  <div class="clearfix mxn1">
    <div class="sm-col sm-col-6 px1">
      ...
      <p if="{ !project.brief.budget }">You have not set a project budget yet (<a href="/api/projects/{ project.id }/brief">add one</a>)</p>
    </div>

    <div class="sm-col sm-col-6 px1">
      <h3>Preferred Start date</h3>
      <p if="{ project.brief.preferred_start }">{ project.brief.preferred_start }</p>
      <p if="{ !project.brief.preferred_start }">You have not defined a start date yet (<a href="/api/projects/{ project.id }/brief">set now</a>)</p>
    </div>
  </div>
  ...
  <p if="{ !hasAnyValue(project.address) }">You have not defined the address yet (<a href="/api/projects/{ project.id }/brief">fix now</a>)</p>
  </div>
  <script>
