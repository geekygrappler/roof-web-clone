...
class RemoteModelSelect extends React.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        return (
            <input
                className={`${this.props.modelName}-search-${this.props.lineItemId}`}
    ...
    fetchRecords() {
        return fetch(`/search/${this.props.modelName}s?item_name=${this.props.itemName}`)
        .then((response) => {
            response.json().then((data) => {
                this.records = data.results;
                $(`.${this.props.modelName}-search-${this.props.lineItemId}`).typeahead(
                    {
                        highlight: true,
                        minLength: 0
                    },
...
RemoteModelSelect.defaultProps = {
    model: {
        name: ""
    },
    ENTER_KEY_CODE: 13,
