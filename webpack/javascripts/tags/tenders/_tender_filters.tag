...
<r-tender-filters>
  <div class="overflow-auto nowrap mb1">
    ...
    <a each="{opts.record.document.sections}" class="btn btn-small {'btn-primary': section == uniqueIdentifier}" onclick="{setSectionFilter}">{name}</a>
  </div>
  ...
  this.setSectionFilter = (e) => {
    this.update({section: e.item ? e.item.uniqueIdentifier : null})
  }
